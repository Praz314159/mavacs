...
fn test_parent_index_calculation_zero_padding() {
    ...
    assert_eq!(MerkleAVC::parent_index_zero_padding(height, 12), Ok(14));

    // Parent of node 13 should be 14
    assert_eq!(MerkleAVC::parent_index_zero_padding(height, 13), Ok(14));

    // Parent of node 10 should be 13
    assert_eq!(MerkleAVC::parent_index_zero_padding(height, 10), Ok(13));

    // Parent of node 11 should be 13
    assert_eq!(MerkleAVC::parent_index_zero_padding(height, 11), Ok(13));
...
fn test_parent_index_calculation_copy_padding() {
    assert_eq!(MerkleAVC::parent_index_copy_padding(2, 5, 0), Ok(6));

    assert_eq!(MerkleAVC::parent_index_copy_padding(5, 3, 5), Ok(8));

    assert_eq!(MerkleAVC::parent_index_copy_padding(7, 3, 5), Ok(9));
}
...
fn test_root_has_no_parent() {
    ...
    let result = MerkleAVC::parent_index_zero_padding(height, 14);
...
fn test_parent_index_out_of_bounds() {
    ...
    let result = MerkleAVC::parent_index_zero_padding(height, 15);
    assert!(result.is_err());
...
fn test_left_child_index_calculation() {
    ...
    assert_eq!(MerkleAVC::left_child_zero_padding(height, 14), Ok(12));

    // Left child of node 13 should be 10
    assert_eq!(MerkleAVC::left_child_zero_padding(height, 13), Ok(10));

    // Left child of node 12 should be 8
    assert_eq!(MerkleAVC::left_child_zero_padding(height, 12), Ok(8));
}
...
fn test_right_child_index_calculation() {
    ...
    assert_eq!(MerkleAVC::right_child_zero_padding(height, 14), Ok(13));

    // Right child of node 13 should be 11
    assert_eq!(MerkleAVC::right_child_zero_padding(height, 13), Ok(11));

    // Right child of node 12 should be 9
    assert_eq!(MerkleAVC::right_child_zero_padding(height, 12), Ok(9));
}
...
fn test_leaf_has_no_children() {
    ...
    let left_result = MerkleAVC::left_child_zero_padding(height, 5);
    assert!(left_result.is_err());
    assert_eq!(left_result.unwrap_err(), TreeIndexError::LeafHasNoChildren);

    let right_result = MerkleAVC::right_child_zero_padding(height, 5);
...
fn test_child_index_out_of_bounds() {
    ...
    // Index 15 is out of bounds (root is 14)
    let left_result = MerkleAVC::left_child_zero_padding(height, 15);
    assert!(left_result.is_err());
    assert_eq!(left_result.unwrap_err(), TreeIndexError::IndexOutOfBounds);

    let right_result = MerkleAVC::right_child_zero_padding(height, 15);
    assert!(right_result.is_err());
...
fn test_build_zero_padded_tree_basic() {
    ...
    let tree = MerkleAVC::build_zero_padding(&data);
    ...
    match tree.stored_values {
        TreeStorageType::Full(ref nodes) => {
...
fn test_build_copy_padded_tree_basic() {
    ...
    let tree = MerkleAVC::build_copy_padding(&data);
...
fn test_generate_copath_first_leaf_zero_padding() {
    ...
    let copath = tree.open_zero_padding(0).unwrap();
...
fn test_generate_copath_middle_leaf_zero_padding() {
    ...
    let copath = tree.open_zero_padding(2).unwrap();
...
fn test_generate_copath_out_of_bounds_zero_padding() {
    ...
    let result = tree.open_zero_padding(5);
...
fn test_verify_copath_valid_zero_padding() {
    ...
    let copath = tree.open_zero_padding(1).unwrap();

    println!("Copath for index 1: {:#?}", copath);

    // Verify the copath with the correct value
    let is_valid = MerkleAVC::verify_zero_padding(
        &copath,
        &tree.root,
        &data[1],
        1,
...
fn test_copath_roundtrip_all_leaves_zero_padding() {
    ...
    for (index, value) in data.iter().enumerate() {
        let copath = tree.open_zero_padding(index as u16).unwrap();


        let is_valid: bool = MerkleAVC::verify_zero_padding(
            &copath,
            &tree.root,
            value,
            index as u16,
...
fn test_generate_copath_first_leaf_copy_padding() {
    ...
    // Generate copath for first leaf (index 0)
    let copath = tree.open_copy_padding(0).unwrap();
...
fn test_verify_copath_valid_copy_padding() {
    ...
    // Generate copath for index 1
    let copath = tree.open_copy_padding(1).unwrap();

    println!("Copath for index 1: {:#?}", copath);

    // Verify the copath with the correct value
    let is_valid = MerkleAVC::verify_copy_padding(
        &copath,
        &tree.root,
        &data[1],
        1,
...
fn test_copath_roundtrip_all_leaves_copy_padding() {
    ...
    // Verify copath for each leaf
    for (index, value) in data.iter().enumerate() {
        let copath = tree.open_copy_padding(index as u16).unwrap();

        println!("index: {}", index);
        println!("copath: {:#?}", copath);

        let is_valid: bool = MerkleAVC::verify_copy_padding(
            &copath,
            &tree.root,
            value,
            index as u16,
